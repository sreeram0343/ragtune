...
from router.domain import IntentCategory, PlanningStrategy, CapabilityMetadata, CapabilityType
...
class IntentDecisionEngine:
    ...
    def select_capabilities_for_intent(
        ...
        permitted_caps = []
        for cap in all_caps:
            if not cap.required_permissions:
                permitted_caps.append(cap)
            elif all(p in user_perms for p in cap.required_permissions):
                permitted_caps.append(cap)
        ...
        if intent == IntentCategory.STRUCTURED_SQL:
            sql_caps = [c for c in permitted_caps if c.type in [CapabilityType.TEXT_TO_SQL, CapabilityType.ANALYTICS_ENGINE]]
            selected.extend(sql_caps)

        elif intent == IntentCategory.UNSTRUCTURED_RAG:
            if strategy == PlanningStrategy.LOW_LATENCY:
                # Fast keyword search
                bm25 = [c for c in permitted_caps if c.type == CapabilityType.RETRIEVAL_BM25]
                ...
                ret_caps = [c for c in permitted_caps if c.type in [CapabilityType.RETRIEVAL_VECTOR, CapabilityType.RETRIEVAL_BM25]]
                selected.extend(ret_caps)
            else:
                # BALANCED: Dense Vector Search
                vec = [c for c in permitted_caps if c.type == CapabilityType.RETRIEVAL_VECTOR]
                selected.extend(vec)

        elif intent in [IntentCategory.HYBRID_ANALYTICS, IntentCategory.RESEARCH]:
            # Both SQL and Vector Retrieval
            hybrid = [c for c in permitted_caps if c.type in [CapabilityType.TEXT_TO_SQL, CapabilityType.RETRIEVAL_VECTOR]]
            selected.extend(hybrid)

        elif intent == IntentCategory.SUMMARIZATION:
            sum_caps = [c for c in permitted_caps if c.type in [CapabilityType.RETRIEVAL_VECTOR, CapabilityType.DOCUMENT_SUMMARIZER]]
