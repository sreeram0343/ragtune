...
class SQLSafetyGuard:
    ...
    def evaluate_sql(self, sql: str, max_limit: int = 100) -> Tuple[bool, float, str, str]:
        """
        ...
        if HAS_SQLGLOT:
            try:
                ...
                if not isinstance(parsed, (exp.Select, exp.Expression)):
                    return False, 0.0, "", "SQL statement is not a valid SELECT expression"
        ...
        return True, 1.0, sanitized_sql, f"SQL safety check passed. Execution safe with limit {max_limit}."
