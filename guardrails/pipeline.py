...
class GuardrailPipeline:
    ...
    def run_pre_execution(
        ...
        evals.append(LayerResult(layer_name="L1: Prompt Injection", layer_num=1, passed=l1_pass, score=l1_score, details=l1_det))

        # Layer 2: PII Masking
        l2_pass, l2_score, masked_query, l2_det = self.l2_pii.evaluate(query)
        evals.append(LayerResult(layer_name="L2: PII Masking", layer_num=2, passed=l2_pass, score=l2_score, details=l2_det, sanitized_output=masked_query))

        # Layer 3: Domain Boundary
        l3_pass, l3_score, l3_det = self.l3_domain.evaluate(masked_query)
        evals.append(LayerResult(layer_name="L3: Domain Boundary", layer_num=3, passed=l3_pass, score=l3_score, details=l3_det))

        # Layer 4: RBAC & Tenant Isolation
        l4_pass, l4_score, l4_det = self.l4_rbac.evaluate_query_permission(user_context, "QUERY_KNOWLEDGE")
        evals.append(LayerResult(layer_name="L4: RBAC Isolation", layer_num=4, passed=l4_pass, score=l4_score, details=l4_det))
        ...
        return PipelineResult(
            ...
            hitl_reason="Pre-execution guardrails violation" if not pre_passed else None
        )
    ...
    def run_post_execution(
        ...
        evals.append(LayerResult(layer_name="L5: Semantic Drift", layer_num=5, passed=l5_pass, score=l5_score, details=l5_det))

        # Layer 6: SQL Execution Safety
        if generated_sql:
            l6_pass, l6_score, safe_sql, l6_det = self.l6_sql.evaluate_sql(generated_sql)
            sanitized_sql = safe_sql
            evals.append(LayerResult(layer_name="L6: SQL Safety", layer_num=6, passed=l6_pass, score=l6_score, details=l6_det, sanitized_output=safe_sql))
        else:
            evals.append(LayerResult(layer_name="L6: SQL Safety", layer_num=6, passed=True, score=1.0, details="No SQL generated"))
        ...
        if raw_response and retrieved_chunks:
            l7_pass, l7_score, l7_det, _ = self.l7_groundedness.evaluate_groundedness(
                raw_response, retrieved_chunks, threshold=settings.GROUNDEDNESS_THRESHOLD
            )
            evals.append(LayerResult(layer_name="L7: Groundedness Verification", layer_num=7, passed=l7_pass, score=l7_score, details=l7_det))
        else:
            evals.append(LayerResult(layer_name="L7: Groundedness Verification", layer_num=7, passed=True, score=1.0, details="Groundedness bypassed (direct response)"))

        # Layer 8: Toxicity & Harm Safety
        l8_pass, l8_score, l8_det = self.l8_toxicity.evaluate(sanitized_resp)
        evals.append(LayerResult(layer_name="L8: Toxicity & Bias", layer_num=8, passed=l8_pass, score=l8_score, details=l8_det))

        # Layer 9: Data Leakage Scanner
        l9_pass, l9_score, l9_det = self.l9_leakage.evaluate(sanitized_resp)
        evals.append(LayerResult(layer_name="L9: Data Leakage Filter", layer_num=9, passed=l9_pass, score=l9_score, details=l9_det))
        ...
        if not all_passed:
            ...
            hitl_reason = f"Post-execution guardrail violation on: {', '.join(failed_layers)}"
        ...
        return PipelineResult(
            ...
            layer_evaluations=evals,
            hitl_triggered=not all_passed or avg_score < settings.HITL_CONFIDENCE_THRESHOLD,
            hitl_reason=hitl_reason or ("Low confidence score trigger" if avg_score < settings.HITL_CONFIDENCE_THRESHOLD else None)
        )
