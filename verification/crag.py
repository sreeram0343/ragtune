...
class CRAGEvaluator:
    def evaluate_evidence_sufficiency(
        self, query: str, source_contexts: list[str], groundedness_score: float
    ) -> tuple[bool, str]:
        """
        ...
        if total_context_len < 30:
            return (
                True,
                "CRAG Triggered: Retrieved evidence text is too short to support reasoning.",
            )

        if groundedness_score < 0.40:
            return (
                True,
                f"CRAG Triggered: Groundedness score ({groundedness_score}) falls below CRAG threshold (0.40).",
            )
