...
class VerificationEngine:
    ...
    def verify_response(
        ...
        custom_risk_override: float | None = None,
    ) -> QualityReport:
        """
        ...
        claims, groundedness_score, citation_coverage = (
            self.grounding_verifier.verify_grounding(
                response_narrative=response_narrative, source_contexts=source_contexts
            )
        )

        # 2. Self-RAG Reflection Tokens
        reflection_tokens = self.self_rag_reflector.reflect(
            query=query, response_narrative=response_narrative, claims=claims
        )
        rel_token = next(
            (t for t in reflection_tokens if t.token_type == "[IS_RELEVANT]"), None
        )
        relevance_score = rel_token.score if rel_token else 0.90

        # 3. Hallucination & Discrepancy Detection
        hallucination_risk, issues = (
            self.hallucination_detector.detect_hallucination_risk(
        ...
        requires_hitl = (security_request.cumulative_risk_score > 40.0) or (
