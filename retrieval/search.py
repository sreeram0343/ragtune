...
class HybridSearchEngine:
    ...
    def search_dense(
        ...
        with self._lock:
            ...
            valid_chunks = [
                ...
                if (c.tenant_id == tenant_id or c.tenant_id == "global_tenant") and
                   (c.workspace_id == workspace_id or c.workspace_id == "global_ws")
            ]

            for chunk in valid_chunks:
                # If chunk vector is missing, compute mock dense score based on text overlap
                sim = _cosine_similarity(query_vector, chunk.embedding) if chunk.embedding else 0.5
                candidates.append((chunk, sim))

            candidates.sort(key=lambda x: x[1], reverse=True)
            results = []
            for rank, (chunk, score) in enumerate(candidates[:top_k], start=1):
                results.append(SearchCandidate(chunk=chunk, score=score, rank=rank, source="DENSE"))
            return results
    ...
    def search_sparse(
        ...
        with self._lock:
            ...
            results = []
            for rank, (chunk, score) in enumerate(candidates[:top_k], start=1):
                results.append(SearchCandidate(chunk=chunk, score=score, rank=rank, source="SPARSE"))
            return results
