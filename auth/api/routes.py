...
from auth.api.schemas import (
    ...
    TokenResponse, PasswordChangeRequest, CreateOrgRequest,
    CreateWorkspaceRequest, CreateInvitationRequest, AcceptInvitationRequest
)
...
def change_password(payload: PasswordChangeRequest, ctx: SecurityContext = Depends(get_security_context)):
...
def create_organization(payload: CreateOrgRequest, ctx: SecurityContext = Depends(get_security_context)):
...
def issue_invitation(
    ...
    if not ctx.org_id:
        raise HTTPException(status_code=400, detail="Organization context required to issue invitations")
...
def accept_invitation(payload: AcceptInvitationRequest, ctx: SecurityContext = Depends(get_security_context)):
    """Accepts an invitation token."""
    success, msg = org_service.accept_invitation(
        accepting_user_id=ctx.user_id,
...
def get_audit_logs(
    ...
    return {"events": [l.model_dump() for l in logs]}
