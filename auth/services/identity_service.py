...
class IdentityService:
    ...
    ) -> tuple[bool, UserDomain | None, str]:
        """
    ...
    def authenticate_user(
        ...
        if ip_address and self.rate_limiter.is_ip_rate_limited(ip_address):
            return (
                False,
                None,
                "Too many login attempts from this IP. Please wait a minute.",
            )
        ...
        if user.locked_until and user.locked_until > now:
            ...
            return (
                False,
                None,
                f"Account is temporarily locked due to multiple failed login attempts. Try again in {mins_left} minute(s).",
            )
        ...
        if user.status == UserStatus.SUSPENDED:
            return (
                False,
                None,
                "Account has been suspended. Please contact enterprise administrator.",
            )
        ...
        if not CryptoService.verify_password(password, user.password_hash):
            ...
            should_lock, lockout_until = self.rate_limiter.calculate_lockout(
                user.failed_login_attempts
            )
            if should_lock and lockout_until is not None:
        ...
        access_token, refresh_token, sess = (
            self.token_service.create_session_and_tokens(
    ...
    def suspend_user(
        self, admin_actor_id: str, target_user_id: str, ip_address: str | None = None
    ) -> tuple[bool, str]:
        ...
        return (
            True,
            f"User '{target_user_id}' has been suspended and all sessions revoked.",
        )
