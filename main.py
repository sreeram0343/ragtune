...
def main():
    parser = argparse.ArgumentParser(
        description="RAGTUNE Enterprise Knowledge Intelligence Platform"
    )
    parser.add_argument(
        "--server",
        action="store_true",
        default=True,
        help="Launch FastAPI Web Gateway Server",
    )
    parser.add_argument(
        "--host", type=str, default=settings.HOST, help="Server host IP"
    )
    parser.add_argument(
        "--port", type=int, default=settings.PORT, help="Server port number"
    )
    parser.add_argument(
        "--seed",
        action="store_true",
        help="Seed enterprise database and sample documents",
    )
    parser.add_argument(
        "--query", type=str, help="Execute a single CLI query through RAGTUNE engine"
    )
    ...
    if args.query:
        from api.main import AgentState, get_default_user_context, orchestrator
    ...
    print(
        f"Starting {settings.APP_NAME} v{settings.VERSION} on http://{args.host}:{args.port}"
    )
