...
def check_http_endpoint(url: str, expected_status: int = 200, timeout: int = 5) -> bool:
    try:
        req = urllib.request.Request(
            url, headers={"User-Agent": "RAGTUNE-SmokeTest/1.0"}
        )
        with urllib.request.urlopen(req, timeout=timeout) as response:
            if response.status == expected_status:
                color_print(
                    "PASS", f"Endpoint {url} returned HTTP {response.status}", "32"
                )
                return True
            else:
                color_print(
                    "FAIL",
                    f"Endpoint {url} returned unexpected status {response.status}",
                    "31",
                )
                return False
...
def verify_system_health(base_url: str):
    color_print(
        "INFO",
        f"Initiating RAGTUNE Production Deployment Verification for {base_url}...",
        "36",
    )
    results = []
    ...
    health_url = (
        f"{base_url.rstrip('/')}/api/v1/health" if "/api" not in base_url else base_url
    )
    ...
    if passed == total:
        color_print(
            "SUCCESS",
            f"All {total} Deployment Verification Checks PASSED cleanly!",
            "32",
        )
        sys.exit(0)
    else:
        color_print(
            "ERROR", f"Deployment verification FAILED ({passed}/{total} passed).", "31"
        )
        sys.exit(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RAGTUNE Production Smoke Verification"
    )
    parser.add_argument(
        "--url", default="http://localhost:8000", help="Base URL of RAGTUNE deployment"
    )
