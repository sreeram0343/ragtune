...
class IntelligentCacheManager:
    def __init__(self, provider: Optional[BaseCacheProvider] = None):
        self.provider = provider if provider else InMemoryLRUCacheProvider(capacity=10000)
        self.semantic_cache = SemanticCacheEngine(similarity_threshold=0.92, max_entries=2000)
    ...
    def get_or_compute(
        ...
        """
        key = TenantCacheKeyBuilder.build_key(tenant_id, workspace_id, namespace, payload)
        ...
        query_text = user_query or (payload.get("query") if isinstance(payload, dict) else None)
        if query_text:
        ...
        def _guarded_compute():
            ...
            if query_text:
                self.semantic_cache.store(tenant_id, workspace_id, query_text, res, tags=tags)
            return res
