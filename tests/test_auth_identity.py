...
def test_user_registration_and_authentication():
    ...
    auth_success, data, msg = identity_svc.authenticate_user(
        email="alice@enterprise.com", password="SecurePassword123!"
    )
    assert auth_success
...
def test_brute_force_lockout():
    ...
    identity_svc.register_user(
        "bob@enterprise.com", "CorrectPassword123!", "Bob Analyst"
    )

    # Perform 5 consecutive failed logins
    for i in range(5):
        success, _, _ = identity_svc.authenticate_user(
            "bob@enterprise.com", "WrongPassword!"
        )
        assert not success

    # 6th attempt with correct password should be locked out
    locked_success, _, msg = identity_svc.authenticate_user(
        "bob@enterprise.com", "CorrectPassword123!"
    )
...
def test_password_change_and_session_invalidation():
    ...
    identity_svc.register_user(
        "charlie@enterprise.com", "OldPassword123!", "Charlie User"
    )
    _, data, _ = identity_svc.authenticate_user(
        "charlie@enterprise.com", "OldPassword123!"
    )
