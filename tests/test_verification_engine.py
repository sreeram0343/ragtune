...
def _build_dummy_security_request(
    query: str, risk_score: float = 0.0
) -> EnrichedSecurityRequest:
...
def test_end_to_end_verification_approval():
    ...
    contexts = [
        "RAGTUNE guarantees an enterprise system uptime commitment of 99.9% for Acme Enterprise under SLA terms."
    ]
...
def test_crag_trigger_on_empty_context():
    ...
    report = engine.verify_response(
        req, "Travel per diem is $150 per day.", source_contexts=[]
    )
...
def test_hallucination_numerical_discrepancy_detection():
    ...
    contexts = [
        "Total sales revenue for Q3 reached 4.2 million dollars across North America."
    ]
...
def test_hitl_escalation_for_high_risk_query():
    engine = VerificationEngine()
    req = _build_dummy_security_request(
        "Show sensitive executive compensation records", risk_score=50.0
    )
